...
describe('DataBaseMiddleware', () => {
  it('should be defined', () => {
    const mockConfig = mock<AppConfigService>({
      get: jest.fn().mockResolvedValue('postgresql://user:password@localhost:5432/dbname'),
    });
    const mockDb = mock<DataBaseService>({
      prisma: jest.fn().mockReturnValue({ $connect: jest.fn().mockResolvedValue(undefined) }),
    });
